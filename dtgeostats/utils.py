...
def fit(cprob, nscore_value):
    ...
    :return:
    """
    x, y = cprob, nscore_value

    def f(t):
        if t <= x.min():
            # return y[np.argmin(x)]
            ylow = y[np.argmin(x)].min()*0.8
            yhigh = y[np.argmin(x)]
            xlow = 0
            xhigh = x.min()
            pow = 2
            return ylow + (yhigh - ylow) * (((t - xlow)/(xhigh-xlow))**pow)
        elif t >= x.max():
            # return y[np.argmax(x)]
            yhigh = y[np.argmax(x)].max()*1.2
            ylow = y[np.argmax(x)]
            xhigh = 1
            xlow = x.max()
            pow = 2
            ...
            intr = si.interp1d(x, y, fill_value='extrapolate')
        return intr(t)
    return f
...
def to_norm(data):
    ...
    return z, cprob, nscore_value


def from_norm(data, cprob, nscore_value, mu, sd):
    """
    ...
    :return:
    """
    print('The dtgeostats.utils.from_norm function is under construction - use with caution...')
    h = fit(cprob, nscore_value)
